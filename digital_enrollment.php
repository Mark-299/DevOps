...
?>
  ...
  <script>
    // Delegate clicks on `.student-open` buttons so it works for elements added later
    document.addEventListener('click', function(e){
      var btn = e.target.closest && e.target.closest('.student-open');
      if (!btn) return;
      e.preventDefault();
      var id = btn.getAttribute('data-id');
      function setVal(idSel, attr){ var el = document.getElementById(idSel); if (el) el.value = btn.getAttribute(attr) || ''; }
      ...
      if (status === 'PENDING') {
        if (subjectsCount <= 0) {
          if (approvalWarning) { approvalWarning.classList.remove('d-none'); }
